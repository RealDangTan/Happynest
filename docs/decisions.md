...
## 2026-08-25 — Phase 11: TEST_DATABASE_URL + skip-guard khi DB unreachable; KHÔNG auto-truncate schema; test_auth bổ marker integration
- Context: (1) Plan 11 §3.1 yêu cầu conftest đọc `TEST_DATABASE_URL` fallback `DATABASE_URL`, fixture "tạo schema sạch mỗi lần chạy", fake chung trong conftest, và skip toàn bộ test DB khi DB không reachable. (2) Thực tế DB test duy nhất hiện có là Supabase DEV DÙNG CHUNG chứa data demo — truncate/drop-schema tự động sẽ xoá data thật và phá luôn `alembic_version`. (3) Rà soát §3.2 phát hiện `test_auth.py` từ Phase 04 KHÔNG có marker `integration` dù 12 test đều chạm DB qua fixture `client` — chúng lọt vào unit suite mặc định, chỉ tình cờ xanh vì máy dev luôn online.
- Decision: (1) Conftest inject `TEST_DATABASE_URL` → `os.environ["DATABASE_URL"]` TRƯỚC mọi import app (pydantic-settings ưu tiên env hơn .env nên toàn bộ engine bind sang test DB) — chuẩn bị sẵn cho test project thứ 2; chưa có thì fallback DATABASE_URL như cũ. (2) Thay auto-wipe bằng: probe `SELECT 1` cached mỗi session (engine tạm NullPool, connect_timeout 5s) + autouse guard function-scoped — test mang marker `integration` mà DB unreachable → pytest.skip("Supabase not reachable…"); seeded_users (session-scoped, chạy trước guard) cũng tự probe-skip bên trong. Cô lập data tiếp tục bằng chiến lược tiền tố external_ref + quarantine phase 09 đã chứng minh. (3) Giữ fakes colocated trong từng file test thay vì gom `fake_llm`/`fake_embedder` chung vào conftest — mỗi file có fake chuyên biệt (script chain, preset theo item, crash-once); conftest bổ sung đúng phần dùng được ngay: `db_session` rollback function-scoped. (4) Thêm `pytestmark = pytest.mark.integration` cho test_auth.py.
- Alternatives rejected: (1) drop/create schema tự động như chữ plan — phá data demo DB dùng chung + alembic_version, phải re-migrate mỗi lần chạy test; (2) sqlite in-memory cho unit — bất khả thi do native PG enum + VECTOR(1536) (đã chốt entry spike S3); (3) gom fake LLM/embedder vào conftest rồi refactor các file đang xanh — churn lớn, lợi ích bằng 0, rủi ro vỡ suite trước deadline.
- Consequence: Ma trận chuẩn hoạt động đủ 3 chế độ: `uv run pytest` = unit offline (29 pass), `pytest -m integration` với DB unreachable = 31 SKIP kèm message rõ (không ERROR), với Supabase active = 31 pass thật (~4 phút); khi user tạo Supabase test project thứ 2 chỉ cần đặt `TEST_DATABASE_URL` + `alembic upgrade head` một lần, không phải sửa code; 12 test auth rời unit suite → số liệu "unit 41" cũ thành 29 unit + 31 integration.

## 2026-08-25 — Phase 12: DoD sweep 10/10 ✅ với run THẬT đầu tiên; frontend placeholder bổ sung; ghi chú ops SECRET_KEY
- Context: (1) Sweep dòng DoD 5 cần "1 run đầy đủ trên 20 rows" — owner XÁC NHẬN chạy thật (chi phí ~22 LLM calls + 22 embeddings qua proxy). (2) OUT-OF-SCOPE rà lại phát hiện `frontend/README.md` placeholder CHƯA tồn tại (plan 12 §3.2 yêu cầu). (3) Dòng DoD 2 yêu cầu "role sai bị 403" nhưng API production KHÔNG có route pm-only thật (mọi route đều pm|operations) — case 403 chỉ tái hiện được qua guard-demo route của test.
- Decision: (1) Chạy run thật `9c6687bc` qua POST /api/analysis/runs trên server boot sạch: completed 22/22 (~4 phút 26 giây, error=null; tổng 22 vì DB còn 2 row trống ngoài 20 row CSV), labels mixed VN-EN, 4 review=True do mệnh đề pii_detected của công thức HITL, 22 embedding `(text-embedding-3-small, 1536)` — giữ lại làm data demo xem UI giai đoạn sau. (2) Tạo `frontend/README.md` placeholder phase B1. (3) Ghi nhận 403 bằng integration test guard-demo (thiết kế Phase 04) + curl live cho phần 200/401 — không tạo endpoint pm-only thừa chỉ để có 403. (4) Báo cáo giai đoạn viết thành file riêng `docs/backend-foundation-report.md` (bảng 10 dòng + số liệu + blocker ops).
- Alternatives rejected: (1) Chỉ dùng bằng chứng test idempotency cho dòng 5 — bỏ lỡ cơ hội lấy số liệu thật (latency/item, phân bố nhãn) cho chương kết quả khóa luận; (2) thêm route pm-only thật vào production API để curl ra 403 — làm bẩn API surface chỉ vì bằng chứng.
- Consequence: Backend foundation CHÍNH THỨC đóng 12/12 phase; data demo 22 feedback đã classify nằm trong DB dev (dọn khi nào cũng được qua Supabase Studio); ops note còn treo: SECRET_KEY vẫn placeholder — BẮT BUỘC rotate trước APP_ENV=prod, và khuyến nghị reset database password Supabase (đã từng lộ trong session debug cũ).

## 2026-08-25 — Khai báo giai đoạn Delivery: mở khóa frontend + 4 nhóm stub + LangGraph HITL
- Context: Backend Foundation đã đóng trọn 12/12 phase (00-index tick hết, run thật `9c6687bc` completed 22/22, spike S4/S5 PASS). Owner yêu cầu khai báo "giai đoạn sau" theo đúng cơ chế `AGENTS.md` ("until a later phase is declared"): mở toàn bộ 3 mục out-of-scope — frontend UI, clustering/insight production code, production LangGraph graph + correction→few-shot loop. Deadline luận văn ~2026-10-22 (~8 tuần), máy dev 8 GB RAM.
- Decision: (1) Sửa `AGENTS.md` §CURRENT PHASE thành "Delivery: Frontend + Pipeline Completion" — in scope: frontend Next.js trong `frontend/` với shadcn/ui preset `b4IdeDqtkJ`; 4 nhóm route stub lên production (`/api/clusters`, `/api/insights`, `/api/reviews|corrections/{id}`, `/api/reports/summary`); production LangGraph HITL graph. Docker VẪN cấm tuyệt đối. (2) Phạm vi chi tiết + phasing do design spec & plan series mới trong `docs/plans/` định nghĩa (đang soạn qua brainstorming) — KHÔNG viết code frontend/backend mới trước khi plan được duyệt. (3) Locked stack ở AGENTS.md không đổi gì cho backend; lựa chọn stack frontend (Next.js template, radix base) đi kèm lệnh init owner cung cấp.
- Alternatives rejected: (1) Chỉ mở frontend, để các stub chờ sau — mất điểm nhấn HITL/LangGraph trong demo bảo vệ và phải mở lại lần nữa tốn một vòng plan; (2) bắt đầu code ngay không cần spec — vi phạm quy ước plan-first của repo, rủi ro scope creep đúng lúc deadline gắt nhất.
- Consequence: Mọi agent phiên sau đọc AGENTS.md sẽ thấy frontend + pipeline completion là mission hiện hành; chuỗi plan mới phải tự trả lời phasing (thứ tự FE vs BE, ranh giới file nếu chạy song song nhiều phiên); ops note SECRET_KEY/reset password vẫn treo từ Phase 12 và phải xử lý trước deploy.

## 2026-08-25 — Viết sớm plans backend `13–16` (lệch lịch just-in-time, theo yêu cầu owner)
- Context: `delivery-execute-plan.md` §1 định các plan chi tiết P2–P4 viết just-in-time ("viết đầu T3/T5/T6" bởi session FE). Owner xem bảng 5 endpoint còn stub và yêu cầu planning toàn bộ phần backend còn lại NGAY, chọn phương án "cả 4 plans 13–16".
- Decision: Viết đủ `13-hitl-langgraph.md`, `14-clusters-api.md`, `15-insights-api.md`, `16-reports-summary.md` ngày 2026-08-25, bám `delivery-design-spec.md` §3 (contracts C1–C6) + §5 (backend slices), đúng khung plan của repo (delivery-execute-plan §3). KHÔNG kèm code production — code chỉ chạy khi từng pha được kích hoạt.
- Alternatives rejected: (1) giữ just-in-time — trái yêu cầu tường minh của owner, mỗi pha phải dừng một vòng viết plan; (2) viết luôn cả code P2 — vi phạm quy ước plan-first ("một phiên = một phase", plan phải được duyệt trước).
- Consequence: Pha P2–P4 kích hoạt được bất cứ lúc nào không phải dừng soạn plan; nếu spec/contract đổi giữa chừng, plan tương ứng sửa TRƯỚC khi thực thi pha đó; cột "Plan chi tiết" bảng §1 delivery-execute-plan cập nhật trỏ tới 4 file này. Plan 14 phải chốt lib clustering chính thức (sklearn spike S4 làm nền) kèm entry decisions riêng khi thực thi vì dep ngoài danh sách §1.

## 2026-08-25 — Phase 13: checkpoint setup chạy background-thread thay vì `await` trực tiếp trong lifespan
- Context: Plan 13 §3.5 viết "lifespan gọi trực tiếp `await saver.setup()`". Boot app thật lẫn TestClient đều nổ `psycopg.InterfaceError` NGAY TRONG lifespan: event loop của uvicorn trên Windows là ProactorEventLoop, async psycopg chỉ chạy trên SelectorEventLoop — quirk S5 (2026-08-24) tái hiện ở TẦNG LIFESPAN chứ không chỉ tầng request. Thêm ràng buộc: connection psycopg bám loop nơi nó được tạo nên không thể dựng saver lúc boot rồi tái dùng xuyên request.
- Decision: (1) `hitl_graph.ensure_checkpointer_ready()` — sync, thread-safe (double-checked lock), tự `asyncio.run` trên SelectorEventLoop RIÊNG, idempotent đúng 1 lần mỗi process, trả bool không raise. (2) Lifespan chỉ spawn daemon thread gọi hàm đó — boot không block ~9s WAN và không đụng Proactor loop. (3) `submit_review()` gọi lại ensure TRƯỚC khi vào selector loop của request: thread boot chưa kịp xong thì request đầu tự đảm bảo (không race nhờ lock); Supabase unreachable → raise `CheckpointUnavailable` → route `/reviews` trả 503. Coroutine `_flow` giữ nhánh phòng thủ `await saver.setup()` khi flag chưa set.
- Alternatives rejected: (1) giữ `await` + try/except — vẫn nổ InterfaceError vì sai LOOP POLICY, không phải lỗi mạng, except không cứu được; (2) dựng AsyncPostgresSaver toàn cục lúc boot dùng chung — connection bám Proactor loop, mọi request sau chết (đúng lỗi kinh điển S5 đã ghi); (3) bỏ hẳn setup lúc boot, chỉ setup per-request — mất ý đồ plan "bảng sẵn sàng sớm", request đầu chậm thêm ~9s vô lý khi server đã online lâu.
- Consequence: log "checkpoint saver setup OK" xuất hiện TRỄ HƠN (từ thread nền, không chặn uvicorn startup); `/api/reviews` có thể trả 503 khi Supabase chập chờn đúng lúc chưa có setup thành công nào (retry được — client thấy detail rõ); chữ "lifespan gọi trực tiếp" trong plan §3.5 được thay bằng cơ chế cùng ý đồ: setup một lần lúc boot, non-fatal cho health-degraded philosophy.

## 2026-08-25 — Phase 13: FeedbackOut bổ sung `ai_issue` + `sentiment`; approve xác nhận KHÔNG ghi dòng log
- Context: (1) Integration test C3 phát hiện response của POST /api/corrections (CorrectionOut = FeedbackOut + correction_recorded) không chứa `sentiment`/`ai_issue` sau khi sửa nhãn — client không thấy kết quả mới nếu không GET lại; FeedbackOut thiết kế từ Phase 05, TRƯỚC khi tồn tại classifier labels. (2) Test ban đầu kỳ vọng action=approve cũng ghi 1 dòng `human_reviews` — nhưng contract C3 quy định side effect chỉ có edit/reject, graph hiện hành đúng hợp đồng.
- Decision: (1) Thêm `ai_issue: AiIssue | None` + `sentiment: Sentiment | None` vào FeedbackOut (enum thật của models, serialize ra value string) — mọi endpoint dùng FeedbackOut tự mang thêm 2 field, không đổi route nào. (2) Giữ nguyên hành vi graph; SỬA TEST theo hợp đồng — approve khẳng định `_reviews_count == []` (cả human_reviews lẫn correction_examples trống), biến assertion này thành bằng chứng graph chạy qua nhiều lần checkpoint-write mà không sinh row thừa.
- Alternatives rejected: (1) CorrectionOut định nghĩa shape riêng đủ nhãn — trùng lặp FeedbackOut, drift schema về sau; (2) bắt client GET lại sau khi sửa — thêm round-trip vô ích, mâu thuẫn chữ "FeedbackOut cập nhật nhãn" của C3; (3) thêm ghi log cho approve cho khớp test cũ — mở rộng scope ngoài contract chỉ để thỏa một assertion do chính executor viết sai.
- Consequence: mọi consumer FeedbackOut nhận thêm 2 field nullable (backward-compatible); hành vi "nhãn không gửi → giữ nguyên" kiểm chứng được ngay trên response; approve không để vết audit trail trong DB (chỉ review_status đổi) — chấp nhận đúng contract, nếu sau này cần audit thì là scope mới phải qua decisions.
