...
| ✅ | GET | `/api/analysis/runs/{run_id}` | pm \| operations | Progress một run: status, processed/total, error summary | ✅ | Cùng trang — `useRunProgress` poll 4s khi running, dừng khi completed/failed |
...
| ✅ | POST | `/api/clusters/run` | pm \| operations | Chạy lại toàn bộ clustering HDBSCAN cosine + LLM naming — idempotent trong 1 transaction (xoá insights cũ → clusters cũ → tạo mới), response C5 `{clusters_upserted, assigned_count, unassigned_count, duration_ms}` (plan 14) | ⬜ | Trang `/clusters` — cần nút trigger run |
| ✅ | GET | `/api/clusters` | pm \| operations | Danh sách cụm theo C1 (`sort=feedback_count\|growth_ratio\|recent`, kèm `sample_feedback_ids` ≤5); chưa từng run → `items: []`. Lưu ý: data demo hiện chưa có nhóm chủ đề thật → hay ra rỗng/noise cao (decisions 2026-08-26, dời evidence P5) | ⬜ | Trang `/clusters` placeholder `frontend/app/(app)/clusters/page.tsx` |
